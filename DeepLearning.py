...
w_h2 = tf.Variable(tf.random_normal([num, num], mean=0.0, stddev=0.05))
b_h2 = tf.Variable(tf.zeros([num]))
...
def model(X, w_h, b_h, w_h2, b_h2, w_o, b_o):
    ...
    h2 = tf.sigmoid(tf.matmul(h, w_h2) + b_h2)
	
    # softmax関数は与えたデータから確率っぽいものを求めてくれます
    # softmax関数も活性化関数です
    return tf.nn.softmax(tf.matmul(h2, w_o) + b_o)

y_hypo = model(x, w_h, b_h, w_h2, b_h2, w_o, b_o)
...
with tf.Session() as sess:
    ...
    y_hypo = model(testData.eval(), w_h, b_h, w_h2, b_h2, w_o, b_o)
